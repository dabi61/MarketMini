...
public class SalaryDAO {
    ...
    public void salaryfind(JTable tbBang, int month){
        try {
            ...
            while(rs.next()){
                ...
                vt.add(calculateSalary(rs.getInt("employee_id")));
    ...
    public double calculateSalaryForAll() {
     ...
     public void getTotalWorkingHoursByEmployee(int month) {  
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                if(hasSalaryDataThisMonth(employee_id)){
                    salary_update(sl);
                }else{
                    salary_insert(sl); 
                }
     ...
     public BigDecimal calculateHourlyWage(int employeeId) {
      ...
      public boolean hasSalaryDataThisMonth(int employeeId) {
        LocalDate currentDate = LocalDate.now();
        String sql = "SELECT COUNT(*) FROM salary s " +
